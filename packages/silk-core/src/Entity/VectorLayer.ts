...
export class VectorLayer implements ILayer {
  ...
  public static create({width, height}: {width: number, height: number}) {
    ...
    const sp = spline.points([[0,0,1], [200, 500, 1], [600, 500, 1], [1000, 1000, 1]].map(([x,y]) => [x,y]))
    const [start, ...points] = sp
    const objectPoints = points.map(([c1x, c1y, c2x, c2y, x, y]: number[]) => ({c1x, c1y, c2x, c2y, x, y}))
    const path = new Path({ start: { x: start[0], y: start[1] }, points: objectPoints, svgPath: spline.svgPath(sp)})
