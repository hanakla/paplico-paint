...
export class Stroke {
  ...
  public points: [x:number, y:number, weight: number][] = []
  ...
  public get splinedPath() {
    ...
    const objectPoints = points.map(([c1x, c1y, c2x, c2y, x, y]: number[]) => ({c1x, c1y, c2x, c2y, x, y}))

    return this._splined =  new Path({ start: {x: start[0], y: start[1]}, points: objectPoints, svgPath})
  }
  ...
  public eachSplinePoint(callback: (point: [x: number, y: number, force: number]) => void) {
