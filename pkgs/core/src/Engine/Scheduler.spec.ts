...
describe('Scheduler', () => {
  it('Resolve transforms', () => {
    const group = visu.createGroupVisually({})
    const nestedGroup = visu.createGroupVisually({})

    const normal = visu.createCanvasVisually({ width: 1, height: 1 })
    const doc = createDocument({ width: 1, height: 1 })

    doc.__internal_AddLayerNode(group, [], 0)
    doc.__internal_AddLayerNode(nestedGroup, [group.uid], 0)
    doc.__internal_AddLayerNode(normal, [group.uid, nestedGroup.uid], 0)

    group.transform.translate = { x: 10, y: 10 }
    nestedGroup.transform.translate = { x: 10, y: 10 }

    const docX = new DocumentContext(doc)

    const s = buildRenderSchedule(
      doc.layerNodes.getResolvedLayerNodes([]),
      docX,
      {
        willRenderingViewport: { left: 0, top: 0, width: 1, height: 1 },
      },
    )

    console.log(s.tasks[1])
  })
...
function documentFactory(layers: VisuElement.AnyElement[]) {
  const doc = createDocument({ width: 1, height: 1 })
  layers.forEach((layer) => doc.__internal_AddLayerNode(layer, [], 0))
