...
const pointNormalize = (p: Point, length = 1) =>
  pointMultiplyScalar(p, length / (pointLength(p) || Infinity))
...
const generateBezier = (
  ...
  if (alpha1 < eps || alpha2 < eps) {
    ...
    if (
      pointDot(handle1, line) - pointDot(handle2, line) >
      segLength * segLength
    ) {
