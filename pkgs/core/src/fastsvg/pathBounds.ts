...
// export function pathBounds(path: string | [cmd: string, ...args: number[]][]):  {
//   // if (!isSvgPath(path)) throw Error('String is not an SVG path.')

//   let parsed = Array.isArray(path) ? path : parseSVGPath(path)

//   if (!parsed.length) return { left: 0, top: 0, right: 0, bottom: 0 }
