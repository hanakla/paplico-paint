...
describe('DocumentSerializer', () => {
  ...
  describe('deserialize', () => {
    ...
    it('should preserve Map objects correctly', () => {
      // Add some test data to maps
      testDocument.layers.set('layer1', {
        id: 'layer1',
        name: 'Test Layer',
        visible: true,
        opacity: 1,
      ...
      testDocument.artObjects.set('art1', {
        id: 'art1',
        type: 'path',
        layerId: 'layer1',
        path: {
      ...
      expect(document.layers.size).toBe(1)
      expect(document.layers.get('layer1')).toEqual(
        testDocument.layers.get('layer1'),
      )
      expect(document.artObjects.size).toBe(1)
      expect(document.artObjects.get('art1')).toEqual(
        testDocument.artObjects.get('art1'),
      )
    })
...
describe('FileIOHelper', () => {
  ...
  beforeEach(() => {
    ...
    mockFile = new File([buffer], 'test.paplico', {
...
describe('DocumentConverter', () => {
  ...
  describe('fromJSON', () => {
    ...
    it('should handle Maps correctly in JSON conversion', () => {
      testDocument.layers.set('test-layer', {
        id: 'test-layer',
        name: 'Test Layer',
        visible: true,
        opacity: 1,
      ...
      expect(restored.layers.get('test-layer')).toEqual(
        testDocument.layers.get('test-layer'),
      )
    })
