...
export interface ProjectFile {
  ...
  /** Index signature for CBOR compatibility */
  [key: string]: any;
}
...
export class DocumentSerializer {
  ...
  static deserialize(data: Uint8Array): {
    ...
    try {
      // CBOR デコード
      projectFile = decodeCbor(data) as unknown as ProjectFile;
    } catch (error) {
  ...
  private static serializeDocument(document: Document): any {
    return {
      ...
      artObjects: document.artObjects,
  ...
  static extractMetadata(data: Uint8Array): ProjectFileMetadata {
    try {
      const projectFile = decodeCbor(data) as unknown as ProjectFile;
