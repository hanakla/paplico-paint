...
export class PaplicoEngine {
  ...
  private setupInputHandlers(): void {
    ...
    this.inputManager.on('strokeUpdate', (event) => {
      if (this.state.tools.activeTool !== 'brush') return;
    ...
    this.inputManager.on('strokeComplete', (event) => {
      if (this.state.tools.activeTool !== 'brush') return;
      console.log('Stroke complete:', event.finalEvent);
  ...
  private handleDrawingStart(event: EnhancedPointerEvent): void {
    ...
    if (
      ...
      console.warn(
        '[Paplico] Previous stroke was not properly ended, forcing cleanup',
      );
      debugState.paplicoEngine.forceStrokeCleanup = {
        wasDrawing: this.webgpuEngine.state.isDrawing,
        hadCurrentStroke: !!this.webgpuEngine.state.currentStroke,
        timestamp: Date.now(),
      };
    ...
    // デバッグ: 新しいストロークの開始点を記録
    debugState.paplicoEngine.newStrokeStart = {
      startPoint: { x: enhancedPoint.x, y: enhancedPoint.y },
      worldPos: { x: worldPos.x, y: worldPos.y },
      canvasPos: { x: event.x, y: event.y },
      timestamp: Date.now(),
    };
  ...
  private handleDrawingEnd(_event: EnhancedPointerEvent): void {
    ...
    if (vectorLayer) {
      ...
      if (currentStroke && currentStroke.points.length > 0) {
        ...
        debugState.stroke.strokeSaveData = {
