...
import { AppearanceError } from '@/engine/exceptions';
...
export class StrokeRenderer implements IAppearanceProcessor {
  ...
  async render(
    ...
    try {
      ...
      // デバッグ: ストロークレンダリング時のパス情報を記録
      if (path.points.length >= 2) {
        const firstPoint = path.points[0];
        const lastPoint = path.points[path.points.length - 1];
        debugState.stroke.currentRenderPath = {
          artObjectId: artObjectId || 'unknown',
      ...
      const strokeTexture =
        appearance.params.brushSettings?.texture || 'pencil';
...
export function createStrokeInstances(
  ...
  for (let i = 0; i < instanceCount; i++) {
    ...
    const randomScale =
      1.0 + (seededRandom() - 0.5) * brushSettings.randomScale;
