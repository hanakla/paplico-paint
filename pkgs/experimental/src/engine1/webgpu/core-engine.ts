...
export const debugState = proxy({
  ...
  webgpuEngine: {
    ...
    ioSurfaceDebug: null as any, // IOSurfaceエラー追跡
  },
  ui: {
    componentCount: 0,
...
export class WebGPUEngine {
  ...
  private currentCanvasTexture: GPUTexture | null = null;
  ...
  async initialize(): Promise<boolean> {
    ...
    this.device.addEventListener('uncapturederror', async (event: any) => {
      ...
      debugState.ui.lastError = `WebGPU uncaptured error: ${errorMessage}`;

      // IOSurfaceエラーの場合は特別に記録
      if (
        errorMessage.includes('IOSurface') ||
        errorMessage.includes('Destroyed texture')
      ) {
        debugState.webgpuEngine.ioSurfaceDebug = {
          ...debugState.webgpuEngine.ioSurfaceDebug,
          uncapturedError: {
            message: errorMessage,
            timestamp: Date.now(),
            eventType: 'uncapturederror',
          },
        };
      }
    });
    ...
    for (const appearance of artObject.appearances) {
      ...
      if (isStrokeAppearance(appearance)) {
        ...
        try {
          ...
          console.error('Fill renderer error:', error);
          throw error;
        }
      }
    }
  ...
  public async renderRegion(
    ...
    // 並行実行を防ぐガード
    if (this.isRendering) {
      console.warn('Render already in progress, skipping frame');
      return;
    }
    this.isRendering = true;

    // 注意: フレーム境界リセットはレンダリング完了後に行う
    ...
    if (targetTexture) {
      renderTextureView = targetTexture.createView({
        label: 'CustomTargetTextureView',
      });
    } else {
      // キャンバステクスチャは使用直前に取得（破棄を防ぐため）
      ...
      try {
        ...
        // 現在のcanvasTextureを保存（renderRegionToTextureで再利用するため）
        this.currentCanvasTexture = canvasTexture;
        renderTextureView = canvasTexture.createView({
        ...
        debugState.webgpuEngine.ioSurfaceDebug = {
          ...
          textureWidth: canvasTexture.width,
          textureHeight: canvasTexture.height,
          textureFormat: canvasTexture.format,
          textureUsage: canvasTexture.usage,
          timestamp: Date.now(),
          doubleAcquisitionFixed: true, // 二重取得問題の修正フラグ
        };
      } catch (error) {
        console.error(
          'Failed to get current texture from WebGPU context:',
          error,
        );
        debugState.webgpuEngine.ioSurfaceDebug = {
    ...
    // コマンド送信前のデバッグ情報
    if (debugState.webgpuEngine.ioSurfaceDebug) {
      debugState.webgpuEngine.ioSurfaceDebug.commandSubmissionStart =
        Date.now();
    }

    try {
      // IOSurfaceテクスチャの二重取得を避ける（既に1524行目で取得済み）

      // デバッグ情報: コマンドバッファー送信前の状態
      if (debugState.webgpuEngine.ioSurfaceDebug) {
        debugState.webgpuEngine.ioSurfaceDebug.preSubmissionState = {
          hasCommandBuffer: !!commandBuffer,
          hasDevice: !!this.device,
          hasQueue: !!this.device?.queue,
      ...
      // デバッグ用: GPU処理の完了を待機（IOSurfaceエラー防止のため一時的に追加）
      // TODO: ストロークつながり問題解決後に削除
      await this.device.queue.onSubmittedWorkDone();

      // 成功時のデバッグ情報
      if (debugState.webgpuEngine.ioSurfaceDebug) {
        debugState.webgpuEngine.ioSurfaceDebug.commandSubmissionSuccess = true;
        debugState.webgpuEngine.ioSurfaceDebug.submitTime = Date.now();
        debugState.webgpuEngine.ioSurfaceDebug.gpuWorkDoneAt = Date.now();
      }
    } catch (error) {
      // エラー時のデバッグ情報
      if (debugState.webgpuEngine.ioSurfaceDebug) {
        debugState.webgpuEngine.ioSurfaceDebug.commandSubmissionSuccess = false;
        debugState.webgpuEngine.ioSurfaceDebug.commandSubmissionError =
          error instanceof Error ? error.message : String(error);
        debugState.webgpuEngine.ioSurfaceDebug.errorDetails = {
          errorMessage: error instanceof Error ? error.message : String(error),
          errorStack: error instanceof Error ? error.stack : undefined,
          canvasSize: this.canvas
            ? `${this.canvas.width}x${this.canvas.height}`
            : 'unknown',
          contextState: this.context ? 'valid' : 'invalid',
          currentCanvasTexture: !!this.currentCanvasTexture,
          errorTimestamp: Date.now(),
        };
      }
      // IOSurfaceエラーの調査のため一時的にエラーを握りつぶす
      console.error('GPU command submission failed:', error);
      this.isRendering = false;
      return;
    }
    ...
    // レンダーターゲットとcanvasTextureをクリア（フレーム境界管理）
    this.currentRenderTargetTexture = null;
    this.currentCanvasTexture = null;

    // フレーム完了後にテクスチャプールをリセット（次フレームでのデータ混入防止）
    if (this.layerCompositor) {
      this.layerCompositor.frameReset();
    }
  ...
  startDrawing(path: VectorPath): void {
    ...
    // 前のストロークIDが残っている場合は警告
    if (this.currentTempStrokeId) {
      console.warn(
        '[WebGPUEngine] Previous temp stroke ID still exists:',
        this.currentTempStrokeId,
      );
      debugState.webgpuEngine.startDrawingCall.warningPreviousId =
        this.currentTempStrokeId;
    }
  ...
  private async renderStrokePipeline(
    ...
    const brushSettings =
      ...
      this.state.strokeSettings.brushSettings;
  ...
  protected async renderRegionToTexture(
    ...
    try {
      ...
      // 既に取得されたcanvasTextureを使用（二重取得を避ける）
      const currentTexture: GPUTexture | null = this.currentCanvasTexture;
      if (!currentTexture) {
        console.warn(
          'Cannot sample canvas: currentCanvasTexture is not available',
        );
        return;
      }
