...
type GetDiffMatchPatch = {
  (required?: false | undefined): DiffPatch | null
  (required: true): DiffPatch
}

let getDiffMatchPatch = function(required?: boolean) {
  ...
  if (!cachedDiffPatch) {
    ...
    if (!instance) {
      ...
      ;(error as any).diff_match_patch_not_found = true
      throw error
    }
    ...
    cachedDiffPatch = {
      ...
      patch: function(txt1, patch) {
        let results = instance.patch_apply(instance.patch_fromText(patch), txt1)
        for (let i = 0; i < results[1].length; i++) {
          if (!results[1][i]) {
            let error = new Error('text patch failed')
            ;(error as any).textPatchFailed = true
          }
        }
        return results[0]
      }
...
const textDeltaReverse = function(delta: Delta) {
  let i
  let l
  let lines
  let line
...
export const reverseFilter = function textsReverseFilter(
