...
import { Document, GroupLayer, LayerTypes } from '../../SilkDOM'
...
export class LayerMoveLayer implements ICommand {
  ...
  async do(document: Document): Promise<void> {
    const sourceLayer = SilkDOMDigger.findLayer(
      document,
    ...
    // Delete layer
    const sourceParent = SilkDOMDigger.findLayerParent(
      document,
      this.move.sourcePath,
      { strict: true }
    )
    ...
    const container =
      this.move.targetGroupPath.length === 0
        ? document
        : SilkDOMDigger.findLayer(document, this.move.targetGroupPath, {
            kind: 'group',
            strict: true,
          })
  ...
  async undo(document: Document): Promise<void> {
    const sourceLayer = SilkDOMDigger.findLayerRecursive(
      document,
    ...
    const sourceContainer =
      this.move.sourceGroupPath.length === 0
        ? document
        : SilkDOMDigger.findLayer(document, this.move.sourceGroupPath, {
            kind: 'group',
            strict: true,
          })
